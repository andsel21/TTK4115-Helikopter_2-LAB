 ...
 * C source code generated on : Wed Sep 23 19:36:22 2026
 *
...
void heli_q8_output0(void)             /* Sample time: [0.0s, 0.0s] */
{
   ...
   *  Sum: '<Root>/Sum1'
   */
  heli_q8_B.Sum = (heli_q8_P.Constant2_Value + u0) + heli_q8_B.Joystick_gain_x;

  /* Gain: '<S4>/Gain' */
  rtb_p_c_c = heli_q8_P.Gain_Gain * heli_q8_B.Sum;
  ...
  /* SignalConversion generated from: '<S4>/Gain3' incorporates:
            ...
            if (fp != (NULL)) {
              ...
              if (fwrite(u, sizeof(real_T), 9 + 1, fp) != 9 + 1) {
                rtmSetErrorStatus(heli_q8_M,
                                  "Error writing to MAT-file INT01.mat");
                return;
              }
  ...
  /* Sum: '<S4>/Sum' */
  heli_q8_B.gamma_dot = rtb_p_c_c - heli_q8_B.PitchCounttorad;

  /* Sum: '<S4>/Sum1' */
  heli_q8_B.Zeta_dot = heli_q8_B.e_c_dot - heli_q8_B.ElevationTransferFcn;
}
...
void heli_q8_initialize(void)
{
  ...
  {
    FILE *fp = (NULL);
    char fileName[509] = "INT01.mat";
    if ((fp = fopen(fileName, "wb")) == (NULL)) {
      rtmSetErrorStatus(heli_q8_M, "Error creating .mat file INT01.mat");
      return;
    }

    if (rt_WriteMat4FileHeader(fp, 9 + 1, 0, "ans")) {
      rtmSetErrorStatus(heli_q8_M,
                        "Error writing mat file header to file INT01.mat");
      return;
    }
...
void heli_q8_terminate(void)
{
  ...
  /* Terminate for ToFile: '<Root>/To File' */
  {
    ...
    if (fp != (NULL)) {
      char fileName[509] = "INT01.mat";
      if (fclose(fp) == EOF) {
        rtmSetErrorStatus(heli_q8_M, "Error closing MAT-file INT01.mat");
        return;
      }

      if ((fp = fopen(fileName, "r+b")) == (NULL)) {
        rtmSetErrorStatus(heli_q8_M, "Error reopening MAT-file INT01.mat");
        return;
      }
      ...
      {
        rtmSetErrorStatus(heli_q8_M,
                          "Error writing header for ans to MAT-file INT01.mat");
      }
...
RT_MODEL_heli_q8_T *heli_q8(void)
{
  ...
  /* External mode info */
  heli_q8_M->Sizes.checksums[0] = (87376878U);
  heli_q8_M->Sizes.checksums[1] = (2350606393U);
  heli_q8_M->Sizes.checksums[2] = (3665921362U);
  heli_q8_M->Sizes.checksums[3] = (1310893909U);
  ...
  heli_q8_M->Sizes.numBlocks = (57);   /* Number of blocks */
  heli_q8_M->Sizes.numBlockIO = (20);  /* Number of block outputs */
  heli_q8_M->Sizes.numBlockPrms = (162);/* Sum of parameter "widths" */
  return heli_q8_M;
}
